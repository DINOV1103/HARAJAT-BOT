"""
Moliya Menejeri - Asosiy mantiqiy kod va Eslatuvchi tizim.
...
from database import (
    ...
    add_debt, get_active_debts, mark_debt_as_paid, get_todays_due_debts,
    add_note, get_notes, delete_note
)

from keyboards import (
    main_menu_keyboard, cancel_keyboard, stats_keyboard,
    debts_keyboard, notes_keyboard, admin_keyboard, debt_type_inline, calendar_keyboard, category_inline_keyboard, debt_calendar_keyboard,
...
# ---------- Avtomatik Qarz Eslatuvchi (Background Task) ----------

async def debt_reminder_scheduler(bot: Bot):
    """Har kuni 1 marta qaytarish muddati kelgan qarzlarni egalariga eslatish."""
    while True:
        try:
            today_str = date.today().isoformat()
            due_debts = get_todays_due_debts(today_str)
            
            for d_id, user_id, name, amount, d_type in due_debts:
                direction = "qaytarib olish (sizga kelishi)" if d_type == "lent" else "qaytarib berish (sizdan ketishi)"
                msg = (
                    f"⏰ <b>BUGUN QARZ MUDDATI KELDI!</b>\n\n"
                    f"👤 <b>Shaxs:</b> {name}\n"
                    f"💵 <b>Summa:</b> {format_amount(amount)}\n"
                    f"📋 <b>Amal:</b> Qarzni {direction} kerak.\n\n"
                    f"<i>Hisobni yopgan bo'lsangiz, 'Qarz daftari' -> 'Faol qarzlar' bo'limidan o'chirib qo'ying.</i>"
                )
                try:
            ...
            logging.error(f"Eslatuvchida xatolik: {e}")
            
        await asyncio.sleep(86400) # 24 soat kutiladi va keyingi kunda tekshiradi
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Qarz qaytarish muddatini kalendardan tanlang va bot o'zi eslatishini kuzating.",
...
async def process_expense_amt(message: Message, state: FSMContext):
    ...
    await message.answer("📁 Ushbu harajat qaysi toifaga tegishli?", reply_markup=category_inline_keyboard("expense"))
...
async def process_income_amt(message: Message, state: FSMContext):
    ...
    await message.answer("📁 Ushbu daromad qaysi toifaga tegishli?", reply_markup=category_inline_keyboard("income"))
...
async def save_transaction_with_cat(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"✅ <b>{status_text}!</b>\n\n💰 Summa: <b>{format_amount(amount)}</b>\n📁 Toifa: {category}\n📝 Izoh: {description}", reply_markup=main_menu_keyboard(is_admin))
...
async def show_stats_menu(message: Message):
    await message.answer("📊 Hisobotlar bo'limi:", reply_markup=stats_keyboard())
...
async def export_to_excel_handler(message: Message):
    ...
    if not rows:
        await message.answer("📥 Bazangiz bo'sh.")
        return
    await message.answer("⏳ Excel hisobotingiz shakllantirilmoqda...")
    ...
    await message.answer_document(document=excel_file, caption="✅ Batafsil oylik hisobotingiz tayyor bo'ldi!")
    if os.path.exists(file_path):
        os.remove(file_path)


# ---------- Qarz Daftari Bo'limi (Muddati bilan) ----------
...
async def process_debt_type_callback(callback: CallbackQuery, state: FSMContext):
    g_type = callback.data.split("_")[2]
    await state.update_data(d_type=g_type)
    
    await state.set_state(Form.waiting_debt_date)
    today = date.today()
    await callback.message.delete()
    await callback.message.answer("📅 Ushbu qarz <b>qachon qaytarilishi kerak?</b> Kalendardan kunni tanlang:", reply_markup=debt_calendar_keyboard(today.year, today.month))
    await callback.answer()
...
# Qarz kalendari navigatsiyasi va tanlash amallari
@router.callback_query(F.data.startswith("dcal_nav_"))
async def debt_cal_navigate(callback: CallbackQuery):
...
async def debt_cal_day_selected(callback: CallbackQuery, state: FSMContext):
    ...
    g_type = data.get("d_type")
    
    if not name or not amount or not g_type:
        await callback.answer("Ma'lumot topilmadi, qaytadan urinib ko'ring.", show_alert=True)
        return
        
    add_debt(callback.from_user.id, name, amount, g_type, selected_date)
    await state.clear()
    await callback.message.delete()
    ...
    await callback.message.answer(
        f"✅ <b>Qarz ro'yxatga olindi va muddat belgilandi!</b>\n\n"
        f"👤 Ism: <b>{name}</b>\n"
        f"💵 Summa: <b>{format_amount(amount)}</b>\n"
        f"📋 Tur: {type_str}\n"
        f"⏰ Qaytarish sanasi: <b>{format_date_human(selected_date)}</b>", 
        reply_markup=main_menu_keyboard(is_admin)
    )
    await callback.answer()
...
async def list_debts(message: Message):
    ...
    if not rows:
        await message.answer("📒 Hozirda faol qarzlaringiz vaqtinchalik mavjud emas.")
        return
    await message.answer("📜 <b>Sizning faol qarzlaringiz va qaytarish muddatlari:</b>")
    for d_id, name, amount, d_type, due_date in rows:
        direction = "Bergan qarzim (menga qaytadi) ➡️" if d_type == "lent" else "Olgan qarzim (qaytarishim kerak) ⬅️"
        date_str = format_date_human(due_date) if due_date else "Belgilanmagan"
        txt = f"👤 <b>{name}</b>\n💵 Summa: {format_amount(amount)}\n📋 Holat: {direction}\n⏰ Qaytarish sanasi: <b>{date_str}</b>"
...
async def ask_note(message: Message, state: FSMContext):
    ...
    await message.answer("✏ ... Eslatmani yozib yuboring:", reply_markup=cancel_keyboard())
